...
def get_video_durations_parallel(video_paths):
    ...
    with ThreadPoolExecutor(max_workers=5) as duration_executor:
        future_to_path = {duration_executor.submit(get_video_duration, path): path for path in video_paths}
        durations = {}
        ...
        for future in as_completed(future_to_path):
            ...
            try:
                ...
                print(f'Duration check for {path} generated an exception: {exc}')
                durations[path] = 10.0
...
def process_video_background(session_id, main_title, main_title_color, moment_titles, start_time, word_spacing, processing_status, upload_folder, processed_folder):
    ...
    try:
        ...
        input_videos = [os.path.join(session_folder, f"video{i}.mp4") for i in range(1, 6)]
        
        # Output paths
        processed_session_folder = os.path.join(processed_folder, session_id)
        os.makedirs(processed_session_folder, exist_ok=True)
        
        concatenated_output = os.path.join(processed_session_folder, "concatenated.mp4")
        final_output = os.path.join(processed_session_folder, "final_output.mp4")
        
        # Step 1: Concatenate videos
        processing_status[session_id].update({'step': 'Concatenating videos...', 'progress': 20})
        ...
        if not os.path.exists(final_output):
            processing_status[session_id] = {'status': 'error', 'error': 'Text overlay processing failed'}
            return
