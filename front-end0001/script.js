...
const mainTitleYInput = document.getElementById("main-title-y");
...
function updateSubmitButton() {
  ...
  const validVideoFiles = uploadedFiles.filter((file) =>
    file.type.startsWith("video/")
  );
  const isFormValid =
    validVideoFiles.length === 5 &&
    document.getElementById("title").value.trim() !== "" &&
    getMomentTitles().every((title) => title.text.trim() !== "");
  ...
  if (!isFormValid) {
    if (validVideoFiles.length !== 5) {
      submitButton.textContent = `Select ${
        5 - validVideoFiles.length
      } more video${5 - validVideoFiles.length === 1 ? "" : "s"}`;
    } else {
...
function getMomentTitles() {
  ...
  for (let i = 0; i < 5; i++) {
    const titleValue =
      document.getElementById(`moment-title-${i}`)?.value || "";
    const match = titleValue.match(/<<(.*)>>\s*(.*)/);
    if (match) {
      titles.push({ text: match[2], color: match[1] });
    } else {
      titles.push({ text: titleValue, color: "#FFFFFF" }); // Default color
    }
  }
  return titles;
}
...
async function handleFormSubmit(e) {
  ...
  const mainTitleValue = document.getElementById("title").value.trim();
  if (!mainTitleValue) {
    alert("Please enter a title for your merged video.");
    return;
  }

  const mainTitle = mainTitleValue;
  const mainTitleColor = "#FFFFFF"; // Default color

  const momentTitles = getMomentTitles();
  if (momentTitles.some((title) => title.text.trim() === "")) {
  ...
  try {
    ...
    updateStatus("processing", "Uploading your videos...");
    ...
    updateStatus("processing", "Starting video processing...");
    ...
    const processData = {
      ...
      main_title_y: parseInt(mainTitleYInput.value, 10),
    };
  ...
  const statusInterval = setInterval(async () => {
    try {
      ...
      if (status.status === "processing") {
        ...
        updateStatus(
          "processing",
          `${status.step} - ${status.progress}% complete`
        );
