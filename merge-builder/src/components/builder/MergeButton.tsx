...
const MergeButton: React.FC = () => {
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      const pollStatus = async () => {
        try {
          const statusResponse = await axios.get(`${API_URL}/status/${session_id}`);
  ...
  const handleReset = () => {
    if (status !== 'idle' && status !== 'completed' && status !== 'error') {
      ...
      const confirmed = confirm('Are you sure you want to cancel the ongoing process?');
      if (!confirmed) {
        return;
      }
    }
    ...
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      {status !== 'idle' && status !== 'completed' && status !== 'error' && (
        <div className="w-full text-center">
          <p className="text-xl font-semibold text-gray-700 mb-2">{status}...</p>
      ...
      {status === 'error' && (
        <div className="text-center">
          <p className="text-xl font-semibold text-red-600 mb-4">An error occurred. Please try again.</p>
          <button
            onClick={handleReset}
